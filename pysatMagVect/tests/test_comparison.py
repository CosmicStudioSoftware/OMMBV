...
class TestComparison():
    ...
    def test_apexpy_v_pymv(self):
        ...
        # get locations presuming the setting is for a fixed geomagnetic latitude
        apex = apexpy.Apex(date=self.inst.date)
        mlats, mlongs = apex.geo2qd(glats, glongs, alts)

        # data['glat'], data['glong'], _ = apex.qd2geo(data['glat'], data['glong'], data['alt'])
        ...
        # calculat mag unit vectors in ECEF coordinates
        ...
        # x, y, z = pysatMagVect.geodetic_to_ecef(data['glat'], data['glong'], data['alt'])
        # data['glat'], data['glong'], data['alt'] = pysatMagVect.ecef_to_geocentric(x, y, z)
        #
        # # test enu and ecef conversions
        # igrf_e = np.array(igrf_e)
        # igrf_n = np.array(igrf_n)
        # igrf_u = np.array(igrf_u)
        # x, y, z = pysatMagVect.enu_to_ecef_vector(igrf_e, igrf_n, igrf_u, data['glat'],
        #                                           data['glong'])
        # igrf_e2, igrf_n2, igrf_u2 = pysatMagVect.ecef_to_enu_vector(x, y, z, data['glat'],
        #                                                             data['glong'])
        # # they check out! :)
        ...
        # plt.plot(eq_long, data['zonal_e'], label='tie_zonal_e', color='r', linestyle='dashed')
        # plt.plot(data['glong'], data['zonal_n'], label='tie_zonal_n', color='k', linestyle='dashed')
        # plt.plot(data['glong'], data['zonal_u'], label='tie_zonal_u', color='b', linestyle='dashed')
